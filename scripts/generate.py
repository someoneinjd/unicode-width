#!/usr/bin/env python3
#
# This file is modified from https://github.com/unicode-rs/unicode-width/blob/master/scripts/unicode.py
#
# Copyright 2011-2022 The C++ Project Developers. See the COPYRIGHT
...
MODULE_FILENAME = "unicode_width.hpp"
"""The filename of the emitted C++ module (will be created in the working directory)"""
...
def fetch_open(filename: str):
    ...
    if not os.path.exists(os.path.basename(filename)):
        print(f"{filename} doesn't exists, start downloading from unicode.org...")
        urllib.request.urlretrieve(
            f"http://www.unicode.org/Public/UNIDATA/{filename}", filename
        )
    try:
...
class Bucket:
    ...
    def try_extend(self, attempt) -> bool:
...
def emit_module(out_name: str, tables: list[Table]):
    """Outputs a C++ module to `out_name` using table data from `tables`.
    If `TABLE_CFGS` is edited, you may need to edit the included code for `lookup`.
    """
...
"""
        ...
        for i, table in enumerate(tables):
            ...
            module.write(
                f"""
// Autogenerated. {subtable_count} sub-table(s). Consult `lookup` for layout info.
constexpr static std::uint8_t TABLE_{i}[] = {{"""
            )
        ...
        module.write(
            """
// Returns the [UAX #11](https://www.unicode.org/reports/tr11/) based width of
// `c` by consulting a multi-level lookup table.
// If `is_cjk == true`, ambiguous width characters are treated as double width;
// otherwise, they're treated as single width.
//
// # Maintenance
// The tables themselves are autogenerated but this function is hardcoded.
// You should have nothing to worry about if you re-run `generate.py`
// (for example, when updating Unicode.)
// However, if you change the *actual structure* of the lookup tables
// (perhaps by editing the `TABLE_CFGS` global in `generate.py`),
// you must ensure that this code reflects those changes.
inline size_t lookup(size_t c, bool is_cjk) noexcept {
    const size_t t1_offset = TABLE_0[c >> 13 & 0xFF];

    // Each sub-table in TABLE_1 is 7 bits, and each stored entry is a byte,
    // so each sub-table is 128 bytes in size.
    const size_t t2_offset = TABLE_1[128 * t1_offset + (c >> 6 & 0x7F)];

    // Each sub-table in TABLE_2 is 6 bits, but each stored entry is 2 bits.
    ...
    const size_t packed_widths = TABLE_2[16 * t2_offset + (c >> 2 & 0xF)];

    // Extract the packed width
    const size_t width = packed_widths >> (2 * (c & 0b11)) & 0b11;

    // A width of 3 signifies that the codepoint is ambiguous width.
    return width == 3 ? (is_cjk ? 2 : 1) : width;
}

template <typename CharT>
std::optional<char32_t> read_utf8(const CharT *&begin) noexcept {
    const std::uint8_t ch0 = *begin++;
    if (ch0 < 0x80)  // 0xxx_xxxx
        return ch0;
    if (ch0 < 0xC0) return std::nullopt;
...
template <typename CharT>
std::optional<char32_t> read_utf16(const CharT *&begin) noexcept {
    const std::uint16_t ch0 = *begin++;
    if (ch0 < 0xD800)  // [0x0000‥0xD7FF]
        return ch0;
    if (ch0 < 0xDC00) {  // [0xD800‥0xDBFF] [0xDC00‥0xDFFF]
        const std::uint16_t ch1 = *begin++;
        if (ch1 >> 10 != 0x37) return std::nullopt;
        return static_cast<char32_t>((ch0 << 10) + ch1 - 0x35FDC00);
    }
    if (ch0 < 0xE000) return std::nullopt;
    // [0xE000‥0xFFFF]
    return ch0;
}
...
std::optional<detail::return_t<CharT>> width(CharT c,
    ...
    if (cp < 0x7F) {
        ...
        return detail::lookup(cp, is_cjk);
    } else {
        return std::nullopt;
    }
}
...
std::optional<detail::return_t<CharT>> width(const CharT *str, size_t len,
    ...
    if constexpr (std::is_same_v<CharT, char32_t> ||
        ...
        size_t ret_val = 0;
        while (str != end) {
            auto opt = detail::read_utf16(str);
            if (!opt) return std::nullopt;
            auto w = width(*opt, is_cjk);
            if (!w) return std::nullopt;
            ret_val += *w;
        }
        return ret_val;
...
def main(module_filename: str):
    ...
    lookup table for character width, and write a C++ module utilizing that table to
    `module_filename`.
    ...
    emit_module(module_filename, tables)
